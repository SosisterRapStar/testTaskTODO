...
class AuthService(AbstractAuthService):
    ...
    async def refresh_token(self, user_id: str) -> None:
        ...
        tokens = await self.api_client.refresh_token(refresh_token=tokens_in_dict['refresh_token'])
        await self.redis_client.set_list(key = f"{user_id}:tokens", data = tokens.model_dump())
