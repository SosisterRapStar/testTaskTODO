...
def get_add_tag_keyboard():
    ...
    markup = ReplyKeyboardMarkup(resize_keyboard=True, keyboard=[[button, button2], [cancel_button]])
    return markup
...
async def set_title(message: Message, state: FSMContext):
    ...
    await message.answer("Введите содержимое заметки", reply_markup=get_cancel_keyboard())
...
async def set_content(message: Message, state: FSMContext):
    ...
    await message.answer("Добавьте теги к заметке, либо сохраните её", reply_markup=get_add_tag_keyboard())
...
async def save_note(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Заметка сохранена!\n"
        f"Заголовок: {user_data['title']}\n"
        f"Содержимое: {user_data['content']}\n"
        f"Теги: {', '.join(tags)}",
...
async def save_tag(message: Message, state: FSMContext):
    ...
    await message.answer(f"Тег '{message.text}' добавлен.", reply_markup=get_add_tag_keyboard())
